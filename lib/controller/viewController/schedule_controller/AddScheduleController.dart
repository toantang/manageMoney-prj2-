...
class AddScheduleController extends GetxController {
  ...
  void checkAndInsertSchedule(BuildContext context) {
    if (checkNameSchedule() == 1 && checkMoneySchedule() == 1 && checkInterval() == 1 && checkInputNote() == 1) {
      this._schedule.nameSchedule = this.nameSchedule;
      this._schedule.sumMoneySchedule = this.moneyForSchedule;
      this._schedule.interval = this.interval;
      this._schedule.dateStartSchedule = this.dateStartSchedule;
      this._schedule.note = this.note;
      this._schedule.stateSchedule = this.stateSchedule;
      this._schedule.progessSchedule = this.progessSchedule;

      this._scheduleController.insertSchedule(this._schedule);

      this._textEditingControllerNameSchedule.clear();
      this._textEditingControllerMoneyForSchedule.clear();
      this._textEditingControllerIntervalSchedule.clear();
      this._textEditingControllerNoteSchedule.clear();

      Navigator.pop(context);
    }
    else {
      showFlushbarError(context, 'add schedule is fail');
    }
  }
